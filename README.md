...
| **밖에서 키 발급** | 키 서버 + `web/admin.html` | **무료.** 발급·이메일·고객 입장 24시간. [만드는 법](./server/README.md) |
| **밖에서 실행까지** | 클라우드 `내주소.onrender.com` | 전부 됨. 디스크 때문에 유료. [올리는 법](./docs/deploy.md) |

**확인용 웹주소에서는 키 발급이 안 됩니다.** 고장이 아닙니다 — 그곳은
파일만 내어 주는 곳이라 파이썬이 돌지 않고, 키 발급은 어딘가에 **쓰는**
동작이라 저장할 자리가 없습니다.

밖에서도 발급하시려면 **[무료 키 서버](./server/README.md)** 를 쓰세요.
구글 앱스 스크립트 + 구글 시트라 돈이 들지 않고, 16종과 공인중개사·maim 이
**한 키 체계**로 묶입니다. 파이썬이 있어야 하는 무거운 실행만 집 컴퓨터에서
하시면 됩니다.
...
Claude 는 **저장소**에 고칩니다. 휴대폰이든 회사든 어느 IP 에서 부르시든
결과는 한 곳에 쌓입니다. 집에 오시면 `tools\home\집컴퓨터_최신화.bat` 한 번
누르시면 받아 옵니다. 자세한 것은 [tools/home/README.md](./tools/home/README.md).

**고객 자료(`data/dashboard.db`)는 집 컴퓨터에만 있습니다.** 저장소에 올라가지
않으니 최신화해도 그대로입니다. 백업은 이 파일 하나만 복사해 두세요.
