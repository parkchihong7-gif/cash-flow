...
function 처음설정() {
  ...
  // 다음 할 일은 **처음 까는 분과 다시 돌린 분이 다르다.**
  //
  // 한 번 데였다. 코드를 고쳐 다시 붙이고 `처음설정` 을 돌린 분께 처음
  // 까는 사람용 «[새 배포]» 를 그대로 냈다. 그대로 누르면 **주소가 바뀌어**
  // 이미 알려 준 주소가 죽고, 판 키가 전부 안 먹는 것처럼 보인다.
  //
  // 비밀번호가 이미 있었다는 것은 전에 한 번 돌렸다는 뜻이다. 그것을
  // 갈림길로 쓴다.
  var 다음 = 새로만듦 ? [
    '  다음 할 일',
