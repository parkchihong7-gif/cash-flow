...
test('다시 돌린 분께는 [새 배포] 를 누르지 말라고 한다', () => {
  // 한 번 데였다. 코드를 고쳐 다시 붙이고 처음설정을 돌린 분께 처음 까는
  // 사람용 «[새 배포]» 를 그대로 냈다. 그대로 누르면 주소가 바뀌어 이미
  // 알려 준 주소가 죽는다.
  // **빈 상태**에서 시작해야 한다. PROPS 에는 비밀번호가 이미 들어 있어서,
  // 그걸 쓰면 첫 호출부터 «다시 돌린» 판정이 난다.
  const srv = makeServer({});

  const 처음 = srv.sandbox.처음설정();
  assert.ok(처음.includes('[새 배포]'), '처음 까는 분께는 이 길이 맞습니다');
  assert.ok(!처음.includes('[배포 관리]'));

  const 다시 = srv.sandbox.처음설정();
  assert.ok(다시.includes('[배포 관리]'), '다시 돌린 분께 갈 길을 안 알려 줍니다');
  assert.ok(다시.includes('주소가 바뀝니다'), '왜 안 되는지를 말해야 합니다');
  // «아직 한 번도 배포 안 하셨으면» 이라는 단서가 붙은 한 줄은 남겨 둔다.
  // 그 경우에는 [새 배포] 가 맞는 길이다.
  assert.ok(!다시.includes('[새 배포] → 유형'),
            '다시 돌린 분께 처음 까는 사람용 차례를 그대로 시키고 있습니다');
  assert.ok(다시.includes('이미 정해 두신 것을 그대로'));
});
