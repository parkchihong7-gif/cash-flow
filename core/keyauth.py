...
#: 이 키로 무엇이 열리는가.
#:
#: **한 번 개념을 잘못 잡았다.** 산 분을 «재판매하는 사람» 으로 보고 «관리자»
#: 라고 불렀는데, 이 상품들을 사시는 분은 되팔려고 사는 게 아니다. **자기
#: 일에 쓰려고** 사신다. 그래서 이름을 바꿨다.
#:
#:   판매용  돈 주고 사신 분. 자기 서버에 세워 **자기가 쓴다**
#:   체험용  사시기 전에 기간을 두고 맛보시는 분. 우리 서버가 열린다
#:
#: 속값(`admin`/`client`)은 그대로 둔다. 이미 판 키의 장부가 그 값으로
#: 적혀 있어서, 바꾸면 그 키들이 전부 못 들어온다.
ROLE_ADMIN = "admin"      # 판매용 — 사신 분. 자기 서버에 세워 자기가 쓴다
ROLE_CLIENT = "client"    # 체험용 — 기간을 두고 맛보시는 분

#: 화면·메일에 보일 이름. **여기 하나만 고치면 다 바뀐다.**
ROLE_LABEL = {ROLE_ADMIN: "판매용", ROLE_CLIENT: "체험용"}
