...
| 13 | `exam-drill/` | 공인중개사 기출 풀이 분석기 | **운영 중** |
| 14 | `senior-video/` | 시니어 영상 비용 견적·절감기 | **운영 중** |
| 15 | `naver-blog/` | 네이버 블로그 초안 생성기 | **운영 중** |
| 16 | `speaker-desk/` | 해외 연사 초청 관리 | **운영 중** |

번호는 대시보드 표시 순서이고 `program.yaml` 의 `number` 로 정합니다.

## 되풀이해 돌려야 하는 상품

대부분은 필요할 때 한 번 돌리고 끝납니다. 아래 셋은 다릅니다.

| 번호 | 상품 | 주기 | 거르면 |
|---|---|---|---|
| 7 | `groupbuy-ledger/` | 공구 건마다 (달 단위) | 정산이 밀려 문의가 온다 |
| 11 | `agency-kit/` | 매주 월요일 | 보고서가 안 가면 리테이너가 흔들린다 |
| 12 | `niche-research/` | **매일 새벽** | 거른 날은 영영 빈다. 유튜브 API 는 지난 값을 안 준다 |

`program.yaml` 의 `schedule:` 에 주기와 크론 한 줄을 적어 두면 대시보드
**🗓 정기 실행** 화면이 밀렸는지 봐 줍니다. 돌리는 것은 cron 이나 n8n 이 하고,
대시보드는 읽기만 합니다.
