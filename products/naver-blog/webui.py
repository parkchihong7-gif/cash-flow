...
def console(program, ctx) -> Console:
    ...
    **두 모드가 같은 주소다.** 무엇을 넣느냐로 갈린다 — 마스터 토큰이면
    [관리자 설정 → 접속 코드 관리] 까지 보이고, 고객 코드면 그 칸이 자동으로
    숨겨진다(maim 의 `refreshAccessCodes`). 1번의 `?admin=1` 과는 다른 방식이라,
    화면이 그 차이를 분명히 말해야 한다.
    """
    ...
    시작 = Tab(
        ...
        intro="본체는 **밖에서 돕니다.** 사장님 컴퓨터가 꺼져 있어도 열립니다. "
              "여기서는 팔고, 코드를 주고, 문의에 답하는 일만 하십니다.",
        panels=[
            Panel(key="where", title="주소는 하나입니다", tone="good",
                  table=Table(
                      headers=["누가", "무엇을 넣고", "무엇이 보이나"],
                      rows=[
                          ["사장님", "마스터 토큰", "전부 — **관리자 설정 → 접속 코드 관리** 포함"],
                          ["고객", "접속 코드 (1차 → 2차)", "접속 코드 관리 칸이 **자동으로 숨겨짐**"],
                      ]),
                  notes=[Note("", f"주소: {주소}", "info"),
                         Note("", "**고객에게도 이 주소를 그대로** 보내시면 됩니다. "
                                  "코드만 따로 드리면 알아서 갈립니다.", "info")]),
            Panel(key="flow", title="파는 순서",
                  lines=[
                      "1. 관리자 화면에서 **1차(초대) 코드**를 발급",
                      "2. 주소와 그 코드를 고객에게 전달 — **지금은 손으로**. 메일 발송은 예정",
                      "3. 고객이 1차 코드를 넣으면 **기기별 2차 코드 3개**가 나옵니다",
                      "4. 기기마다 하나씩 넣고 쓰십니다",
                  ]),
        ])
    ...
    코드 = Tab(
        ...
        admin_only=True,
        intro="[관리자 설정 → 4) 접속 코드 관리] 에서 합니다. **마스터 토큰으로 "
              "들어갔을 때만** 그 칸이 보입니다.",
        panels=[
            Panel(key="two", title="두 단계",
                  table=Table(
                      headers=["단계", "몇 개", "쓰임"],
                      rows=[
                          ["1차 (초대)", "1개", "고객에게 주는 것. 넣으면 2차가 나옵니다"],
                          ["2차 (기기별)", "3개", "PC · 노트북 · 휴대폰. 이것으로 로그인"],
                      ]),
                  notes=[Note("", "**1차·2차 모두 한 번만** 쓸 수 있습니다. "
                                  "동시에 여러 번 넣어도 하나만 성공합니다.", "info")]),
            Panel(key="mail", title="메일 발송 — 아직 없습니다", tone="warn",
                  lines=[
                      "지금은 관리자 화면에 코드가 뜨고, **직접 전달**하십니다",
                      "메일로 자동 발송하는 기능은 **구축·연동 예정**입니다",
                      "1번(공인중개사)에 쓰는 방식을 그대로 붙일 수 있습니다 — "
                      "구글 앱스 스크립트 + 시트 장부, 서버 비용 0원",
                  ],
                  notes=[Note("", "붙이기 전까지는 코드를 화면에서 복사해 "
                                  "카톡·메일로 보내시면 됩니다.", "info")]),
            Panel(key="reset", title="전부 다시 발급",
                  lines=[
                      "[기존 접속 코드 10개를 모두 폐기하고 새로 발급] 이 있습니다",
                      "**이미 나눠 준 코드가 즉시 무효가 됩니다.** 쓰시기 전에 "
                      "그 코드를 받은 분이 있는지 보세요",
                  ]),
        ])
            ...
            Panel(key="qa", title="자주 들어오는 것",
                  table=Table(
                      ...
                      rows=[
                          ...
                          ["코드를 잃어버렸어요",
                           "1차·2차 모두 한 번만 씁니다. 새로 발급해 드려야 합니다"],
                          ["**관리자 설정에 접속 코드 관리가 없어요**",
                           "정상입니다. **고객 코드로 들어오면 숨겨집니다**"],
                          ["기기를 바꿨습니다",
                           "그 기기용 2차 코드가 남아 있으면 그것으로. 셋 다 썼으면 재발급"],
                      ])),
        ])
    ...
    return Console(
        ...
        todos=[
            ...
            Todo("새 고객에게 접속 코드 보내기", tab="codes", admin_only=True,
                 by_hand=True,
                 detail="관리자 설정 → 접속 코드 관리에서 1차 코드를 복사해 "
                        "**직접** 전달하십니다. 메일 발송은 아직 없습니다."),
            Todo("들어온 문의에 답하기", tab="ask",
                 detail="«접속 코드 관리가 안 보인다» 가 제일 많습니다 — 정상입니다."),
        ],
        manual_tasks=[
            ManualTask(
            ...
            ManualTask(
                task="접속 코드를 전달하는 것",
                where="카톡·메일 — 손으로",
                why="**메일 발송이 아직 붙어 있지 않습니다.** 관리자 화면에 뜬 "
                    "코드를 복사해 보내십니다.",
                someday="1번에 쓰는 앱스 스크립트 + 시트 방식을 그대로 붙이면 "
                        "서버 비용 없이 자동 발송이 됩니다. 예정입니다."),
            ManualTask(
        ...
        troubles=[
            Trouble("관리자 설정에 «접속 코드 관리» 가 안 보인다",
                    "**고객 코드로 들어오신 것입니다. 정상입니다.** 마스터 토큰으로 "
                    "들어가야 보입니다. 주소는 같습니다."),
            ...
            Trouble("재배포했는데 반영이 안 된다",
                    ...
                    "남아 있는 경우가 잦습니다."),
            Trouble("고객이 코드를 잃어버렸다",
                    "1차·2차 모두 한 번만 씁니다. 재발급해 주셔야 합니다. "
                    "[전부 다시 발급] 은 **이미 나눠 준 것까지** 무효가 되니 주의하세요."),
        ],
        files=[
            FileLoc("프로그램 본체", 주소,
        ...
        admin_intro="**본체는 밖에서 돕니다.** 관리자도 고객도 같은 주소로 들어가고, "
                    "무엇을 넣느냐로 갈립니다 — 마스터 토큰이면 접속 코드 관리까지 "
                    "보이고, 고객 코드면 그 칸이 숨겨집니다.",
